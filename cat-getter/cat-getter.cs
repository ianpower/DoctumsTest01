    ...
    class Program
    {
        static async Task Main(string[] args)
        {
            ...
            using (HttpClient client = new HttpClient())
            {
                ...
                // Loop to make 20 API calls
                int successfullApiCalls = 0;
                int maxAttempts = 20;
                for (int i = 0; i < maxAttempts; i++)
                {
                    try
                    {
                        ...
                        //If the response is not null, add it to the results list
                        if(result != null)
                        {
                            results.Add(result);
                            successfullApiCalls++;
                ...
                Console.WriteLine($"Total successfull calls: {successfullApiCalls}/{maxAttempts}");
        ...
        {
            // Define site credentials
            string ftpHost = "ftp://laria-net.com";
            string ftpUsername = "doctums.dev.tests@laria-net.com";
            string ftpPassword = "pN7[Fv2oIoTa"; // Parameterize for production
